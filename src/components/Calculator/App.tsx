...
export default function Calculator() {
  ...
  console.log(`result: ${result}, pendingOperator: ${pendingOperator}, isWaitingForOperand: ${isWaitingForOperand}, display: ${display}, memory: ${memory}`)

  const calculate = (rightOperand: number, pendingOperator: arithmeticOperator) => {
      ...
      <div className="w-[320px] h-[480px] flex flex-col outline outline-1 outline-black/80">
        <div className="h-1/4 bg-neutral-600/90 ">
          <Display
            display={display}
            hasMemory={memory !== 0}
            expression={pendingOperator !== undefined ?
              `${result} ${pendingOperator} ${isWaitingForOperand ? "" : display}` : ""}
          />
        </div>
        <div className="h-3/4">
          <Pad
